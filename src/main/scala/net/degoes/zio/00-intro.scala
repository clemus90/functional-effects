...
object ZIOModel {
  ...
  final case class ZIO[-R, +E, +A](run: R => Either[E, A]) { self =>
    ...
    def flatMap[R1 <: R, E1 >: E, B](f: A => ZIO[R1, E1, B]): ZIO[R1, E1, B] =
      ZIO(r => run(r).map(f).flatMap(_.run(r)))
...
object PrintSequenceZip extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (putStrLn("Christian") zip putStrLn("Lemus") zip putStrLn("Gonzalez")).exitCode
}
...
object PrintSequence extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("Christian") *> putStrLn("Lemus") *> putStrLn("Gonzalez").exitCode
}
...
object SimpleDuplication extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = {
    ...
    putStrLn("Hello") *> helloAgain *> helloAgain *> helloAgain.exitCode
  }
}
...
object NumberGuesser extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    for {
      answer <- nextIntBounded(5)
      _      <- putStrLn("Guess a number I'm thinking")
      guess  <- readLine
      _      <- analyzeAnswer(answer, guess)
    } yield ExitCode.success
}
