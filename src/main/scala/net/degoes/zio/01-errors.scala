...
object ErrorNarrowing extends App {
   ...
   * an `IOException`:
   */
  val myReadLine: IO[IOException, String] = broadReadLine.refineToOrDie
...
object AlarmApp extends App {
  ...
  lazy val getAlarmDuration: ZIO[Console, IOException, Duration] = {
    def parseDuration(input: String): IO[NumberFormatException, Duration] =
      ZIO.effect(input.toLong).refineToOrDie[NumberFormatException].map(_.seconds)

    def fallback(input: String): ZIO[Console, IOException, Duration] =
      putStrLn(s"${input} is not a valid duration") *> getAlarmDuration
...
object Sandbox extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    composed.sandbox
      .foldM(
        cause => putStrLn(cause.prettyPrint),
        _ => ZIO.unit
      )
      .exitCode
}
